...
This is NOT a nice thing to find when working on a ticket, and you need to alter it. It messes with velocity
and is becoming a negative wild card due to its complexity. Complex decision-making combined with complex mocking
does not make things easy. It has now got to the point that even people who believe in doing test first are 
forced to do test last, making mockery of the process. The final D in TDD is also about design. We are not designing,
we are hacking.
